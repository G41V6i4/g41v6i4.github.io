...
fastbin/smallbin/unsortedbin/largebin 을 사용한다.
...
- request size가 fastbin최대 크기보다 작다면 fastbin 루틴을 실행한다.
- 크기를 통해 fastbin의 index를 구한다
...
- 재할당된 청크에 대한 검사를 하는 [매크로](https://g4ng1a.tistory.com/12)를 실행한다.
...
csize2tidx로 인덱스를 가져온 후 tc_idx에 저장한다.
...
- bck : bin->bk->bk
- bin의 bk가 bck의 fd와 같지 않으면 에러를 출력한다
- bin->bk에 inuse bit를 설정한다
- bin->bk를 bck로 덮고 bck->fd를 bin으로 덮는다
- 재할당된 청크에 대한 검사를 하는 [매크로](https://g4ng1a.tistory.com/entry/glibc229-checkremallocedchunk)를 실행한다.
